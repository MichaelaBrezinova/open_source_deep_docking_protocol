...
#### Example
An example target receptor (receptor.pdbqt) with configuration file (conf.txt) required by Vina and parameter file (logs.txt) are available in **results** directory (TODO). Example output after one iteration for DD-prepared library (filtered by molecular weight <=360) is available at XXX(TODO). 
