...
parser.add_argument('-full_morgan_path','--full_morgan_path',required=True,help='Full path to morgan directory')
...
full_morgan_path = io_args.full_morgan_path
...
for f in part_files:
    with open(SAVE_PATH + '/iteration_' + str(n_it) + '/simple_job_predictions/simple_job_' + str(ct) + '.sh', 'w') as ref:
        ...
        ref.write('#SBATCH --time=02:00:00\n')
        ...
        ref.write('python -u ' + 'Prediction_morgan_1024.py' + ' ' + '-fn' + ' ' + f.split('/')[
            -1] + ' ' + '-protein' + ' ' + protein + ' ' + '-it' + ' ' + str(n_it) + ' ' + '-mdd' + ' ' + str(full_morgan_path) + ' ' + '-file_path' + ' ' + SAVE_PATH + '\n')
        ref.write("\n echo complete")
